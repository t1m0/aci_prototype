...
from Application.Paparazzi import Paparazzi
...
class PaparazziSimulation:
            ...
            for diagonal_movement in self.diagonal_movement_options:
                ...
                for heuristic in selected_heuristics:
                    ...
                    for iteration in range(iterations):
                        ...
                        paparazzi = Paparazzi(map.startpoint)
                        total_iteration = 0
                        time_elapsed = 0
                        ...
                        path = []
                        a_star_executions = 0
                        while(self.__is_not_finished(paparazzi,map)):
                            path, a_star_executions = self.__update_path_if_required(paparazzi,map,heuristic,path,a_star_executions,smart_path_finding,diagonal_movement)
                            next_cell = self.__find_next_move(paparazzi.get_current_cell(), path)
                            paparazzi.move(next_cell)
                            map.move_security_guards()
                            total_iteration += 1
                            time_elapsed = self.__time_elapsed(start_time)
                            intermediate_info_count = self.__plot_current_state(map, paparazzi, iteration_name, time_elapsed, total_iteration, intermediate_info_count)
                        ...
                        plot_map(map,paparazzi.path)
                        result = PathFindingResult(iteration_name, map.name, str(heuristic), paparazzi.path, a_star_executions, total_iteration, time_elapsed, diagonal_movement, num_security_guards, smart_path_finding)
    ...
    def __update_path_if_required(self,paparazzi, map, heuristic, path, a_star_executions, smart_path_finding,diagonal_movement):
        if not path or map.security_guard_in_close_proximity(paparazzi.get_current_cell()) or not smart_path_finding:
            path, total_cost, a_star_iterations = A_Star(map, paparazzi.get_current_cell(), heuristic, diagonal_movement)
            a_star_executions += 1
    ...
    def __is_not_finished(self, paparazzi:Paparazzi, map:Map):
        return paparazzi.get_current_cell() != map.endpoint
    
    def __plot_current_state(self, map, paparazzi, iteration_name, time_elapsed, total_iteration, intermediate_info_count):
        plot_name = iteration_name + "_" + str(total_iteration) + ".png"
        plot_map(map, paparazzi.path, save_image=True, plot_name=plot_name, only_plot_current_position=True)
        if (time_elapsed > 15) and (not self.__is_not_finished(paparazzi,map)):
